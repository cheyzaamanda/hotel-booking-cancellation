...
# ══════════════════════════════════════════════════════════════════
# PAGE 6 — TUTORIAL DEPLOYMENT
# ══════════════════════════════════════════════════════════════════
elif page == "📖 Tutorial Deployment":
    st.markdown("# 📖 Tutorial Deployment Streamlit")
    st.markdown("**Step-by-step dari Colab → file lokal → deploy ke cloud**")
    st.markdown("---")

    st.info("📌 Tutorial ini mengasumsikan kamu sudah selesai training model di Google Colab dan ingin men-deploy dashboard ini secara online.")

    steps = {
        "Step 1: Persiapan File dari Google Colab": {
            "desc": "Unduh semua artifact yang diperlukan dari Colab ke komputer lokal.",
            "code": """# Jalankan ini di cell terakhir Colab
...
print("Semua file berhasil didownload!")""",
            "notes": ["Pastikan Section 6 (Save Model Artifact) sudah dijalankan dulu", "Simpan semua file .pkl di folder yang sama dengan app_streamlit.py"]
        },
        "Step 2: Setup Folder Proyek": {
            "desc": "Buat struktur folder yang rapi di komputer lokal.",
            "code": """hotel-booking-dashboard/
├── app_streamlit.py       ← file utama dashboard
├── best_model.pkl         ← model dari Colab
├── scaler.pkl             ← scaler dari Colab
├── feature_names.pkl      ← nama fitur dari Colab
├── model_metadata.json    ← metadata model
├── requirements.txt       ← list library
└── README.md              ← deskripsi proyek (opsional)""",
            "notes": ["Semua file harus dalam 1 folder yang sama", "Jangan taruh file pkl di subfolder"]
        },
        "Step 3: Buat requirements.txt": {
            "desc": "File ini memberitahu Streamlit Cloud library apa yang harus diinstall.",
...
lightgbm>=4.0.0
joblib>=1.3.0""",
            "notes": ["Versi bisa disesuaikan — cek versi di Colabmu dengan !pip show nama_library", "Jangan taruh library yang tidak dipakai"]
        },
        "Step 4: Install Streamlit di Lokal (Testing)": {
            "desc": "Uji dashboard di komputer dulu sebelum deploy ke cloud.",
            "code": """# Di terminal / command prompt:
...
streamlit run app_streamlit.py""",
            "notes": ["Browser akan otomatis terbuka di http://localhost:8501", "Jika tidak terbuka, buka manual di browser", "Ctrl+C untuk menghentikan"]
        },
        "Step 5: Upload ke GitHub": {
            "desc": "Streamlit Cloud membutuhkan repo GitHub sebagai sumber kode.",
            "code": """# Di terminal, dari folder proyek:
...
git add .
git commit -m "Initial commit: hotel booking dashboard"

# 4. Buat repo baru di github.com (nama: hotel-booking-dashboard)
# Lalu push:
git remote add origin https://github.com/USERNAME/hotel-booking-dashboard.git
git branch -M main
git push -u origin main""",
            "notes": ["File pkl boleh di-push karena ukurannya kecil (<100MB)", "Buat repo PUBLIC agar bisa deploy gratis di Streamlit Cloud", "Ganti USERNAME dengan username GitHub kamu"]
        },
        "Step 6: Deploy ke Streamlit Cloud": {
            "desc": "Deploy gratis menggunakan Streamlit Community Cloud.",
            "code": """Langkah di browser:

1. Buka: https://share.streamlit.io
2. Login dengan akun GitHub
3. Klik "New app"
4. Isi form:
   - Repository: USERNAME/hotel-booking-dashboard
   - Branch: main
   - Main file path: app_streamlit.py
5. Klik "Deploy!"
6. Tunggu 2-5 menit

Dashboard akan online di:
https://USERNAME-hotel-booking-dashboard-app-streamlit-XXXXX.streamlit.app""",
            "notes": ["Gratis untuk repo public", "Otomatis redeploy setiap kali kamu push ke GitHub", "Bisa share linknya ke dosen/penguji langsung!"]
        },
    }

    for step_title, content in steps.items():
        with st.expander(f"📌 {step_title}", expanded=("Step 1" in step_title)):
            st.markdown(f"**{content['desc']}**")
            st.code(content['code'], language="bash" if "git" in content['code'] or "pip" in content['code'] else "python")
            if content['notes']:
    ...
    st.markdown("---")
    st.markdown("### 🚨 Troubleshooting Umum")
    troubles = {
        "❌ ModuleNotFoundError": "Tambahkan library yang hilang ke requirements.txt, lalu push ulang ke GitHub",
        "❌ FileNotFoundError best_model.pkl": "Pastikan file pkl ada di folder yang sama dengan app_streamlit.py",
        "❌ Streamlit version error": "Coba hapus versi spesifik di requirements.txt, ganti dengan streamlit>=1.20.0",
        "❌ Memory limit exceeded": "Kurangi ukuran model atau gunakan caching @st.cache_resource",
        "❌ App crashes on startup": "Cek log di Streamlit Cloud — biasanya import error atau missing file",
    }
    for err, sol in troubles.items():
        st.markdown(f"""<div style='background:#1a0a0a;border-radius:8px;padding:10px;margin:5px 0;
        border-left:3px solid #ef5350;'>
        <b>{err}</b><br><small style='color:#aaa;'>→ {sol}</small>
        </div>""", unsafe_allow_html=True)

    st.markdown("---")
    st.success("🎉 **Setelah berhasil deploy, kamu bisa share URL Streamlit ke dosen sebagai bukti deployment UAS!**")
